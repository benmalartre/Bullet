...
void BTGetWorldBoundingBox(btPhysicsSdkHandle sdk,BTVector3 bb_min,BTVector3 bb_max)
{
    ...
    memcpy(bb_min, physicsSdk->m_worldAabbMin, sizeof(btVector3));
    memcpy(bb_max, physicsSdk->m_worldAabbMax, sizeof(btVector3));
...
btConstraintHandle BTNewConeTwistConstraint(btRigidBodyHandle bodyA,btRigidBodyHandle bodyB,BTXForm* frameA,BTXForm* frameB)
{
	...
	void* mem = btAlignedAlloc(sizeof(btSliderConstraint),16);
	btConeTwistConstraint* coneTwist = new(mem) btConeTwistConstraint(*A,*B,fA,fB);

	return (btConstraintHandle)coneTwist;
}
...
btCollisionShapeHandle	BTNewBvhTriangleMeshShape(int num_tri,int* indices,int num_vertices,BTReal* vertices, bool aligned)
{
    int vertStride;
    if(aligned)vertStride = 4*sizeof(BTReal);
    else vertStride = 3*sizeof(BTReal);
    ...
    m_indexVertexArrays = new (mem)btTriangleIndexVertexArray(num_tri,
                                                              indices,
                                                              indexStride,
                                                              num_vertices,
                                                              vertices,
...
int	BTRayCast(btDynamicsWorldHandle world, const BTVector3 rayStart, const BTVector3 rayEnd, btRayCastResultHandle resHandle)
{
    ...
    btRayCastResult *res = reinterpret_cast<btRayCastResult *>(resHandle);
